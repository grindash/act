...
#' @return data frame
#' @export 
malia_zeta <- function(data, siclex=NULL, tau_diffus=0.54, output = "global", pattern=NULL, data_frame=F) {
  sky <- unique(data$sky)
  ...
  if (nrow(data) > 0) {
    ...
    if (nb_direct > 0) {
      rc_direct_brut <- as.numeric(data[data$ray == "direct", "rc_malia"])
      rs_direct_brut <- as.numeric(data[data$ray == "direct", "rs_malia"])
      par_direct <- 1  # MALIA ne fait que ciel clair
    } else {
      rc_direct_brut <- 0
      rs_direct_brut <- 0
      par_direct <- 0
    ...
    if (nb_diffus > 0) {
      rc_diffus_brut <- as.numeric(data[data$ray == "diffus", "rc_malia"])
      rs_diffus_brut <- as.numeric(data[data$ray == "diffus", "rs_malia"])
      par_diffus <- 0
    } else {
      rc_diffus_brut <- 0
      rs_diffus_brut <- 0
    ...
    if (data_frame) {
      output <- cbind(data, malia_zeta = output)
      output$ray <- NULL
    }
